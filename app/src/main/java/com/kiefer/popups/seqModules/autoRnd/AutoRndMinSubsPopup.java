...
public class AutoRndMinSubsPopup extends SubsPopup {
    private final AutoRandomModule autoRandomModule;

    public AutoRndMinSubsPopup(LLPPDRUMS llppdrums, SequenceModule sequenceModule, final AutoRandomModule autoRandomModule, final Step step, final ImageView stepIV) {
    ...
    public View.OnClickListener getListener(FrameLayout subLayout, int sub){
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                new AutoRndMinPopup(llppdrums, sequenceModule, autoRandomModule, subIV, step, sub, AutoRndMinSubsPopup.this, subLayout);
            }
        };
    }
    ...
    public void setSubLayout(FrameLayout layout, Step step, int sub){
        layout.setBackground(new AutoRndSliderSubsDrawable(llppdrums, ((step.isOn() && step.isSubOn(sub)) || step.getAutoRndOn(sub)) && sequenceModule.getAutoRndOn(step, sub), sequenceModule.getAutoRndMin(step, sub)));
    }
}
