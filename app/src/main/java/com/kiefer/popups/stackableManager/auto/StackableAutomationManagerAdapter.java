package com.kiefer.popups.stackableManager.auto;
 ...
 */
public class StackableAutomationManagerAdapter extends AutomationManagerAdapter{
    private final StackableManagerPopup fxManagerPopup;
    private final int floatMultiplier;
    private CSpinnerButton paramBtn;

    public StackableAutomationManagerAdapter(LLPPDRUMS llppdrums, StackableManagerPopup fxManagerPopup) {
        super(llppdrums);
    ...
    public void onBindViewHolder(final AutomationViewHolder autoViewHolder, final int position) {
        final AutomationManager automationManager = fxManagerPopup.getStackableManager().getSelectedStackable().getAutomationManager();
        ...
        paramBtn.getButton().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new AutomationParamsListPopup(llppdrums, automationManager, StackableAutomationManagerAdapter.this, automationIndex, paramBtn);
            }
        });
        ...
        autoViewHolder.stepsLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new AutomationStepsPopup(llppdrums, automationManager, StackableAutomationManagerAdapter.this, automationIndex, autoViewHolder.stepsLayout);
            }
        });
    ...
    public int getItemCount() {
        if(fxManagerPopup.getStackableManager().getSelectedStackable() != null) {
            return fxManagerPopup.getStackableManager().getSelectedStackable().getAutomationManager().getAutomations().size();
        }
        else{
            return 0;
        }
    ...
    public void moveAutomation(int from, int to){
        fxManagerPopup.getStackableManager().getSelectedStackable().getAutomationManager().moveAutomation(from, to);
    }
}