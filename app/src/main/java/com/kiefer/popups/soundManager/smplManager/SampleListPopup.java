...
public class SampleListPopup extends Popup {
    ...
    public SampleListPopup(LLPPDRUMS llppdrums, final SoundManager soundManager, final CSpinnerButton btn){
        super(llppdrums);
        this.btn = btn;

        //inflate the View
        final View popupView = llppdrums.getLayoutInflater().inflate(R.layout.popup_list, null);
        popupView.findViewById(R.id.listBgIV).setBackground(ContextCompat.getDrawable(llppdrums, soundManager.getSmplManager().getSampleListImageId()));
        ...
        ArrayList<Sample> samples = soundManager.getSmplManager().getSelectedCategory().getSamples();
        ...
        for(int i = 0; i < samples.size(); i++){
            ...
            tv.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    soundManager.getSmplManager().setSample(finalI);
                    soundManager.getSmplManager().getSelectedCategory().setSelectedSample(finalI);
